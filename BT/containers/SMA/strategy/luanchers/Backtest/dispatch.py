...
fs.split(file="/appcode/spiderdoc/BT/Symbols", split_size=size_per_file, output_dir="/appcode/spiderdoc/BT/containers/SMA/tmp/", newline=True)
...
for sym_suffix_host in range(1,len(active_hosts)+1):
    large_file_path='/appcode/spiderdoc/BT/containers/SMA/tmp/Symbols_'+str(sym_suffix_host)
    size_per_file = int((os.stat(large_file_path).st_size) / int(container_amnt))+8
    fs.split(file=large_file_path, split_size=size_per_file, output_dir="/appcode/spiderdoc/BT/containers/SMA/tmp/", newline=True)

#start backtest.py on all active hosts at the same time with appropriate args
i=1
for host in active_hosts:
    p = multiprocessing.Process(target=cmd_over_ssh, args=(host,'python3 /appcode/spiderdoc/BT/containers/SMA/strategy/luanchers/Backtest/backtest.py Symbols_'+str(i)+' '+start_date+' '+end_date+' '+run_type+' '+container_amnt+' '+prallel_proc_amnt))
    Pros.append(p)
    p.start()
